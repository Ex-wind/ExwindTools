...
local function RegisterMiniToolsRefresh(callback)
    MiniToolsRefreshCallbacks[#MiniToolsRefreshCallbacks + 1] = callback
end
...
local function Init_ShowMapInfo()
    ...
    RegisterMiniToolsRefresh(function()
        ApplyFont()
        ApplyAnchor()
    end)
...
-- ========================================================================
-- ========================================================================
-- 4. [AutoInsertKeystone] 打开大秘境面板时自动放入钥石
-- ========================================================================
local function Init_AutoInsertKeystone()
    local retryToken = 0
    local function IsEnabled()
        local db = ExwindTools:GetModuleDB(EXWIND_MODULE_KEY)
        return db and db.AutoInsertKeystone == true
    end
    local function FindKeystoneInBags()
        if not C_Container or not C_Container.GetContainerNumSlots or not C_Container.GetContainerItemInfo or not C_Item or not C_Item.IsItemKeystoneByID then return
            nil, nil end
        for bag = _G.BACKPACK_CONTAINER or 0, _G.NUM_TOTAL_EQUIPPED_BAG_SLOTS or _G.NUM_BAG_SLOTS or 4 do
            for slot = 1, C_Container.GetContainerNumSlots(bag) or 0 do
                local itemInfo = C_Container.GetContainerItemInfo(bag, slot)
                if itemInfo and itemInfo.itemID and C_Item.IsItemKeystoneByID(itemInfo.itemID) then
                    if C_ChallengeMode and C_ChallengeMode.CanUseKeystoneInCurrentMap and ItemLocation and ItemLocation.CreateFromBagAndSlot then
                        ...
                        if location and C_ChallengeMode.CanUseKeystoneInCurrentMap(location) then return bag, slot end
                    else
                        return bag, slot
                    end
                end
    ...
    local function TryAutoInsertKeystone()
        if not IsEnabled() or not C_ChallengeMode or not C_ChallengeMode.SlotKeystone or not C_ChallengeMode.HasSlottedKeystone or C_ChallengeMode.HasSlottedKeystone() or (CursorHasItem and CursorHasItem()) then return false end
        if C_MythicPlus and C_MythicPlus.GetOwnedKeystoneLevel and not C_MythicPlus.GetOwnedKeystoneLevel() then return false end
    ...
    ExwindTools:RegisterEvent("CHALLENGE_MODE_KEYSTONE_RECEPTABLE_OPEN", "ExTools_Mini_AutoInsertKeystone", function()
        if not IsEnabled() then return end
        retryToken = retryToken + 1
        local token = retryToken
        local function Attempt(remaining)
            if token ~= retryToken or not IsEnabled() or TryAutoInsertKeystone() then return end
            if remaining > 0 then C_Timer.After(.15, function() Attempt(remaining - 1) end) end
        end
        Attempt(8)
    end)
    ExwindTools:RegisterEvent("CHALLENGE_MODE_KEYSTONE_SLOTTED", "ExTools_Mini_AutoInsertKeystone", function()
        retryToken = retryToken + 1
    end)
end

-- ========================================================================
-- 5. [AutoCombatLog] 自动战斗记录
-- ========================================================================
local function Init_AutoCombatLog()
    local function CheckLog()
        ...
        if (dID == 16 or dID == 233) and db.ACL_RaidMythic then shouldLog = true end
...
local function Init_BulkBuy()
    ...
    -- 该面板直接挂载在 MerchantFrame 旁；整套 UI 已加载时只加自身背景，
    -- 不调用 ElvUI / NDui / EllesmereUI 的皮肤 API。
    local function ApplyBulkBuyBackdrop(frame)
...
local function Init_HideBattleTag()
    ...
    RegisterMiniToolsRefresh(Apply)
...
SafeInit(Init_AutoInsertKeystone, "AutoInsertKeystone")
