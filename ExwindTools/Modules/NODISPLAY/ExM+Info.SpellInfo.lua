...
local function EXSP_BuildDungeonIconMap()
    ...
    for _, mapID in ipairs(mapIDs) do
        ...
        elseif mapName and mapName ~= "" then
...
local function EXSP_GetDungeonMetaByName(dungeonName)
    if not EXDB or not EXDB.GetInstanceNoteMetaByName then
        return nil
    end
    return EXDB:GetInstanceNoteMetaByName(dungeonName)
end

local function EXSP_GetLocalizedDungeonDisplayName(dungeonName)
    local meta = EXSP_GetDungeonMetaByName(dungeonName)
    if meta and EXDB and EXDB.GetLocalizedInstanceNoteName then
        return EXDB:GetLocalizedInstanceNoteName(meta)
    end
...
local function EXSP_GetLocalizedMobDisplayName(dungeonName, mobName)
    ...
    if dungeonMeta and EXDB and EXDB.GetEncounterNoteMetaByMapIDAndName and EXDB.GetLocalizedEncounterNoteName then
        ...
        if encounterMeta then
            return EXDB:GetLocalizedEncounterNoteName(encounterMeta)
        end
    end
    return L[mobName] or mobName
end
        ...
        local slider = ExwindTools.UI:CreateSlider(
            ...
            function(value) -- onValueChanged callback
                ...
                if value ~= _G.EXMD.EX_DB.mythicLevel then
                    ...
                    EXUI:NotifyModuleValueChanged("ExM+.MythicDamage", "mythicLevel", "committed")
                end
            end
        )
...
function EXSP_RefreshMobList(dungeonName, filter)
    ...
    for _, name in ipairs(sorted) do
        ...
        if not filter or name:lower():find(filter) or displayName:lower():find(filter) then
            ...
            --@@ 左侧怪物名字体大小 (19)
            b.nameText:SetFont(EXSP.CurrentFont, 19, "OUTLINE"); b.nameText:SetText(displayName)
            b:SetScript("OnClick",
                function()
